...
the definition is silent. A `_you` form (declared, or the engine's own for keys that
name a player) addresses the person at this screen.
...
- **Scoring summaries.** The round-score lines ("Player 1: +12 = 40") are composed in
  the scoring engine. Every other message and button on the table goes through `text`.
