        ...
        foreach (var zoneDef in state.Definition.Zones)
        {
            if (zoneDef.Owner == "each_player")
            {
                foreach (var p in state.Players)
                {
                    ...
                    state.Zones[id] = new Zone(id, zoneDef.Type, p.Id, zoneDef.Visibility, zoneDef);
                }
            }
            else if (zoneDef.Owner == "each_team")
            {
                ...
                if (state.Teams.Count > 0)
                {
                    foreach (var team in state.Teams)
                    {
                        ...
                        state.Zones[id] = new Zone(id, zoneDef.Type, team.Id, zoneDef.Visibility, zoneDef);
                    }
                }
                else
                {
            ...
            else
            {
                state.Zones[zoneDef.Id] =
                    new Zone(zoneDef.Id, zoneDef.Type, zoneDef.Owner, zoneDef.Visibility, zoneDef);
            }
        }
    }
}
